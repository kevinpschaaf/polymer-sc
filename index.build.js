...
return node.__dom && node.__dom.firstChild ? this._getFirstElementChild(node) : node.firstElementChild;
},
...
return node.__dom && node.__dom.lastChild ? this._getLastElementChild(node) : node.firstElementChild;
},
...
return node.__dom && node.__dom.nextSibling ? this._getNextElementSibling(node) : node.nextElementSibling;
},
...
return node.__dom && node.__dom.previousSibling ? this._getPreviousElementSibling(node) : node.previousElementSibling;
},
...
if (root) {
root._invalidInsertionPoints = this._maybeAddInsertionPoint(node, this.node);
this._addNodeToHost(root.host, node);
}
if (TreeApi.Logical.hasChildNodes(this.node)) {
TreeApi.Logical.recordInsertBefore(node, this.node, ref_node);
}
return this._maybeDistribute(node) || this._tryRemoveUndistributedNode(node);
},
...
;Polymer.AppDocumentScrollBehavior = function () {
return {
...
return eventKey === keyCombo.key && (!keyCombo.hasModifiers || !!event.shiftKey === !!keyCombo.shiftKey && !!event.ctrlKey === !!keyCombo.ctrlKey && !!event.altKey === !!keyCombo.altKey && !!event.metaKey === !!keyCombo.metaKey);
}
...
;Polymer({
is: 'app-header',
behaviors: [Polymer.AppScrollEffectsBehavior],
properties: {
condenses: {
type: Boolean,
value: false
...
observers: ['_condensesChanged(condenses, isAttached)'],
_height: 0,
_deltaHeight: 0,
_topbar: null,
_top: 0,
...
var top = Math.min(this._headerDy, Math.max(0, this.reveals ? this._top + scrollTop - this._lastScrollTop : scrollTop));
...
return this._lastScrollTop >= scrollTop && scrollTop > this._deltaHeight && (!this.reveals || this.condenses);
},
isOnScreen: function () {
...
this.transform('translate3d(0,' + Math.min(y, this._deltaHeight) + 'px, 0)', this._topbar);
}
},
...
;Polymer({
is: 'carbon-route',
behaviors: [Polymer.CarbonRoutingBehavior]
});
;Polymer({
is: 'paper-spinner',
...
;Polymer({
is: 'sc-page-search',
properties: { tracks: { observer: '_tracksChanged' } },
bool: function (q) {
return !!q;
},
uri: function (q) {
...
observers: ['_valueChanged(value, track)'],
listeners: { track: '_tracking' },
...
return url && url.replace(/large.jpg$/, 't300x300.jpg');
},
...
config: {
value: function () {
return { clientId: '15def7b4f6aef65c45731e88afdffb22' };
}
},
user: { value: null },
trackid: { value: null }
...
_sdkLoaded: function () {
var token = localStorage.getItem('polymer-sc-oauth');
this._initSdk(token);
if (token) {
this._getUser();
}
...
redirect_uri: window.location.origin + window.location.pathname.replace(/(.*\/)[^\/]*/, '$1callback.html'),
oauth_token: token
});
},
_getUser: function () {
